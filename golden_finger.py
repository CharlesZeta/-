...
class Tranche:
    def __init__(self, issue, guar, PR, fee):
        self.issue = issue; self.guar = guar
        self.PR = PR;       self.fee = fee
...
def sim_scene(Sfin, vv, seed):
    ...
    for k in range(N):
        if KO:
            ...
            KO=True; ev=k; rA[k]=capA*100; rB[k]=capB*100
        else:
...
def plot_scene(fig, rect, t, S, ap, bp, ev, title, legend=False):
    ...
    ax.tick_params(axis='y', colors='#101066')
...
class GoldenFingerApp(QMainWindow):
    ...
    def _build_left(self):
        ...
        title_box.setStyleSheet('''
            QFrame { background: rgba(255,255,255,0.07);
                     border-radius:8px; padding:6px; border:none; }
        ''')
        ...
        combo_style = '''
            QComboBox {
                background:#0F3460; color:white; border:1px solid #3A6EA5;
                border-radius:5px; padding:4px 8px; font-size:11px;
            }
            QComboBox:hover { border:1px solid #64B5F6; }
            QComboBox QAbstractItemView {
                background:#0F3460; color:white; selection-background-color:#1565C0;
            }
        ...
        '''
        lbl_style = 'color:#E3F2FD; font-size:11px; background:transparent;'

        vb.addWidget(section('① 模块'))
        self.dd_mod = QComboBox()
        self.dd_mod.addItems(['1-Payoff情景', '2-3D曲面', '3-双轴对比', '4-动画+Delta'])
        self.dd_mod.setCurrentIndex(3)
        self.dd_mod.setStyleSheet(combo_style); vb.addWidget(self.dd_mod)

        vb.addWidget(section('② 档位'))
        self.dd_tr = QComboBox()
        self.dd_tr.addItems(['仅A', '仅B', 'A+B对比'])
        self.dd_tr.setCurrentIndex(2)
        self.dd_tr.setStyleSheet(combo_style); vb.addWidget(self.dd_tr)

        vb.addWidget(section('③ 情景'))
        self.dd_sc = QComboBox()
        self.dd_sc.addItems(['下跌→250', '上涨→350', '敲出→395', '随机'])
        self.dd_sc.setCurrentIndex(2)
        self.dd_sc.setStyleSheet(combo_style); vb.addWidget(self.dd_sc)

        vb.addWidget(section('④ 速度'))
        self.lbl_sp = QLabel('速度: 5.0')
        self.lbl_sp.setStyleSheet(lbl_style); vb.addWidget(self.lbl_sp)
        self.sld_sp = QSlider(Qt.Horizontal)
        self.sld_sp.setRange(10, 100); self.sld_sp.setValue(50)
        self.sld_sp.setStyleSheet(slider_style)
        self.sld_sp.valueChanged.connect(lambda v: self.lbl_sp.setText(f'速度: {v/10:.1f}'))
        vb.addWidget(self.sld_sp)

        vb.addWidget(section('⑤ 波动率 σ'))
        self.lbl_vol = QLabel('σ = 0.170')
        self.lbl_vol.setStyleSheet(lbl_style); vb.addWidget(self.lbl_vol)
        self.sld_vol = QSlider(Qt.Horizontal)
        self.sld_vol.setRange(5, 40); self.sld_vol.setValue(17)
        self.sld_vol.setStyleSheet(slider_style)
        ...
        self.btn_run.setStyleSheet('''
            QPushButton {
                ...
                border-radius:7px; padding:10px; border:none;
            }
        ...
        btn_clr.setStyleSheet('''
            QPushButton {
                background:rgba(255,255,255,0.10); color:#CFD8DC;
        ...
        sep.setStyleSheet('color:rgba(255,255,255,0.15); background:rgba(255,255,255,0.15); '
        ...
        info = QLabel(
            ...
            '<span style="color:#B0BEC5">S₀ = K = 300 &nbsp;│&nbsp; L = 375</span><br>'
            '<span style="color:#B0BEC5">T = 3年 &nbsp;│&nbsp; r = 4.5%</span><br><br>'
            '<span style="color:#A5D6A7"><b>Tranche A</b>: 上限 +14.7%</span><br>'
            '<span style="color:#90A4AE">&nbsp; 发行1000 &nbsp;保本1000</span><br>'
            '<span style="color:#90A4AE">&nbsp; PR 70% &nbsp; 费 2.8%</span><br><br>'
            '<span style="color:#FFF176"><b>Tranche B</b>: 上限 +16.45%</span><br>'
            '<span style="color:#90A4AE">&nbsp; 发行1100 &nbsp;保本1000</span><br>'
            '<span style="color:#90A4AE">&nbsp; PR 130% &nbsp;费 4%</span><br>'
        ...
        author_box = QFrame()
        author_box.setStyleSheet('background:rgba(255,215,0,0.10); border-radius:6px; border:none;')
        ab = QVBoxLayout(author_box); ab.setContentsMargins(6,6,6,6); ab.setSpacing(3)

        al0 = QLabel('◆  Authors  ◆')
        al0.setStyleSheet('color:#FFD700; font-size:11px; font-weight:bold; background:transparent;')
    ...
    def _show(self, fig):
        ...
        self.canvas = canvas; self.toolbar = tb
        canvas.draw()
    ...
    def _mod1(self, sc, tr, sp, vol):
        ...
        ax1.axhline(fA0, ls='--', color=COL_A, lw=1.0)
        ax1.axhline(fB0, ls='--', color=COL_B, lw=1.0)
        ...
        ax1.set_title(f'(a) 看涨期权 Payoff  A:+{payoff_call(375,P.A)*100:.1f}%  '
                      f'B:+{payoff_call(375,P.B)*100:.1f}%',
    ...
    def _mod2(self, *_):
        ...
        S_a = np.linspace(260,420,60); V_a = np.linspace(0.05,0.40,50)
        Sg, Vg = np.meshgrid(S_a, V_a)
        ...
        ax.set_title('模块2: 机构利润曲面  (鼠标拖拽旋转 / 工具栏缩放)',
    ...
    def _mod4(self, sc, sp, vol):
        ...
        if KO>=0:
            ...
            for k in range(KO,N):
                if k>KO:
                    iPnL+=deltas[k-1]*(Sp[k]-Sp[k-1])/300*0.08+P.r*dt*0.3
                preI[k]=iPnL*100
        ...
        axR.axhline(capA,ls=':',color=COL_A,lw=1)
        axR.axhline(capB,ls=':',color=COL_B,lw=1)
        ...
        self._show(fig)  # 先绑定 canvas

        # 保存所有动画相关对象，防止被 GC 或失效
        self._anim_lines = {
            'lnP': lnP, 'lnBal': lnBal,
            'lnRA': lnRA, 'lnRB': lnRB, 'lnRI': lnRI,
            'lnDlt': lnDlt, 'lnReb': lnReb,
        }
        self._anim_d = dict(
            tp=tp, Sp=Sp, preA=preA, preB=preB, preI=preI,
            deltas=deltas, rebal=rebal, N=N,
            KO=KO, axP=axP, ko_done=False
        )
        self._anim_k = 1
        self._timer.start(max(8, int(80/sp)))
    ...
    def _anim_step(self):
        try:
            d = self._anim_d
            L = self._anim_lines
            if d is None or not L or self.canvas is None:
            ...
            L['lnP'].set_data(tp, Sp)
            L['lnBal'].set_data([tp[-1]], [Sp[-1]])
            L['lnRA'].set_data(tp, d['preA'][:k+1])
            L['lnRB'].set_data(tp, d['preB'][:k+1])
            L['lnRI'].set_data(tp, d['preI'][:k+1])
            L['lnDlt'].set_data(tp, d['deltas'][:k+1])
            L['lnReb'].set_data(tp, d['rebal'][:k+1])

            if d['KO'] >= 0 and k == d['KO'] and not d['ko_done']:
                d['ko_done'] = True
                d['axP'].plot(tp[-1], 375, 'p', ms=26, mfc='red', mec='k', mew=2, zorder=10)
                d['axP'].text(tp[-1]+0.04, 381, '⚡ 敲出!', color='red',
